...
CreditManager.prototype.showClientModal = function(clientId) {
    ...
    if (isEssepi && client.transactions) {
        for (var i = 0; i < client.transactions.length; i++) {
            ...
            if (t.description.toLowerCase() === 'acconto') {
                serviziValue += Math.abs(t.amount) * 0.10;
            }
        }
    }
    
    var finalBalance = balance - serviziValue;
    var balanceColor = finalBalance < 0 ? 'var(--danger)' : (finalBalance > 0 ? 'var(--success)' : 'var(--text-secondary)');
    var serviziHTML = isEssepi && serviziValue > 0 ? 
        '<div class="client-total"><span>Servizi (10%):</span><span class="total-value" style="color: var(--danger);">- ' + formatter.currency.format(serviziValue) + '</span></div>' : '';
    ...
    if (client.transactions && client.transactions.length > 0) {
        ...
        transactionsHTML = sortedTransactions.map(function(t) {
            var amountColor = t.amount > 0 ? 'var(--success)' : 'var(--danger)';
            var amountSign = t.amount > 0 ? '+' : '-';
            return '<div class="transaction-item">' +
                '<div class="transaction-details">' + t.description + 
                ...
                amountSign + formatter.currency.format(Math.abs(t.amount)) + '</div>' +
                '<button class="transaction-delete-btn" onclick="window.creditManager.deleteTransaction(\'' + clientId + '\', \'' + t.id + '\')">' +
                '<i class="fa-solid fa-xmark"></i></button>' +
                '</div></div>';
        }).join('');
    } else {
        transactionsHTML = '<p class="no-transactions-message">Nessuna transazione.</p>';
    }
    
    var clientNameHTML = '<input type="text" class="client-name-input" value="' + client.name + '">';
    var moreOptionsIcon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M6 10c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2m12 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2m-6 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2"/></svg>';
    
    var disabledAttr = finalBalance >= 0 ? 'disabled' : '';
    
    modalBody.innerHTML = 
        '<div class="client-header">' + clientNameHTML + 
        '<div class="client-actions-wrapper">' +
        '<div class="client-actions">' +
        '<button class="client-acconto-btn" title="Acconto"><i class="fa-solid fa-euro-sign"></i></button>' +
        '<button class="client-salda-btn" title="Salda" ' + disabledAttr + '><i class="fa-solid fa-check"></i></button>' +
        '<button class="client-print-btn" title="Stampa"><i class="fa-solid fa-print"></i></button>' +
        '<button class="client-delete-btn" title="Elimina"><i class="fa-solid fa-trash"></i></button>' +
        '</div>' +
        '<button class="client-more-options-btn" title="Altro">' + moreOptionsIcon + '</button>' +
        '</div></div>' +
        ...
        '<span class="total-value" style="color: ' + balanceColor + '">' + formatter.currency.format(finalBalance) + '</span>' +
        '</div>';
...
CreditManager.prototype.bindModalEvents = function(clientId, modal) {
    ...
    content.querySelector('.client-more-options-btn').addEventListener('click', function(e) { 
        content.querySelector('.client-actions').classList.toggle('is-open'); 
    });

    var nameInput = content.querySelector('.client-name-input');
    if (nameInput) {
        nameInput.addEventListener('focus', function() {
            var actions = content.querySelector('.client-actions');
