...
const VenditeManager = {
    ...
    renderRevenueCompositionChart: function(data) {
        ...
        this.renderChart('islandChart', 'bar', {
            ...
            datasets: [
                { label: 'Servito', data: products.map(p => data.revenueByProductAndModality[p].servito), backgroundColor: '#2196F3', borderRadius: 4 },
                { label: 'Iperself', data: products.map(p => data.revenueByProductAndModality[p].iperself), backgroundColor: '#FF4444', borderRadius: 4 },
                { label: 'Self-Service', data: products.map(p => data.revenueByProductAndModality[p].selfservice), backgroundColor: '#00BCD4', borderRadius: 4 }
            ]
        }, {
            responsive: true,
            maintainAspectRatio: false, // MODIFICATO: Permette al grafico di riempire il contenitore
